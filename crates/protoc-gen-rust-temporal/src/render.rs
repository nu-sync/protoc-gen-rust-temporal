...
pub fn render(svc: &ServiceModel, options: &crate::options::RenderOptions) -> String {
    ...
    // Header includes the plugin's Cargo version so consumers can
    // spot at-a-glance which plugin build produced the file (faster
    // than reading `<module>::PLUGIN_VERSION`, which requires opening
    // a Rust scope). Matches the convention `protoc-gen-go` and most
    // other code generators use. Embedded via env! so the version
    // tracks the Cargo manifest.
    let _ = writeln!(
        out,
        "// Code generated by protoc-gen-rust-temporal v{}. DO NOT EDIT.",
...
fn render_start_options(out: &mut String, wf: &WorkflowModel) {
    ...
    if !defaults.is_empty() {
        ...
        let _ = writeln!(out, "            self");
        let _ = writeln!(out, "        }}");
        // `fill_proto_defaults` — in-place mutating sibling of
        // `with_proto_defaults(self) -> Self`. Same underlay
        // semantics; mutates instead of consuming. Pairs with
        // `merge_in` (mutating sibling of `merge`) so the
        // chain/in-place pair pattern is consistent across the
        // StartOptions builder surface.
        let _ = writeln!(
            out,
        ...
        let _ = writeln!(out, "        pub fn fill_proto_defaults(&mut self) {{");
        if wf.id_reuse_policy.is_some() {
            let _ = writeln!(out, "            if self.id_reuse_policy.is_none() {{");
            let _ = writeln!(
                out,
...
fn render_handle(out: &mut String, svc: &ServiceModel, wf: &WorkflowModel) {
    ...
    // sibling handles on the same client without re-fetching from
