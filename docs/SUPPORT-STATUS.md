...
| `search_attributes` | supported (slices 1 + 2 + 3 string/int/bool/double/float refs) | The empty-map `root = {}` (slice 1), non-empty literal maps `root = { "Key": <literal>, … }` (slice 2) with string / signed-integer / boolean / IEEE-754 double values, and `this.<field>` references targeting singular `string` / `int64` / `bool` / `double` / `float` input fields (slice 3) compile and flow through to `WorkflowStartOptions.search_attributes`. The field-ref encoder reads from the start path's `input` binding at call time; `float` fields widen via `as f64`. Double literals are recognised by the lexer when they contain `.` or `e`/`E`; NaN / infinity are refused at parse so the render-side `.expect(…)` stays load-bearing for compile-time literals and surfaces a runtime panic only for genuinely non-finite input field values. Repeated / map / message / enum fields and other scalar types (`int32`, `uint64`, `bytes`, …) plus `typed_search_attributes` remain rejected. See `docs/R7-BLOBLANG.md`. |
