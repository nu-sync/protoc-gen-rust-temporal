...
| `search_attributes` | supported (slices 1 + 2 + 3 — full primitive matrix) | The empty-map `root = {}` (slice 1), non-empty literal maps `root = { "Key": <literal>, … }` (slice 2) with string / signed-integer / boolean / IEEE-754 double values, and `this.<field>` references targeting singular input fields of these proto kinds: `string`; `int64` / `sint64` / `sfixed64` (direct); `int32` / `sint32` / `sfixed32` / `uint32` / `fixed32` (widened to i64); `bool`; `double` (direct); `float` (widened to f64). The field-ref encoder reads from the start path's `input` binding at call time. Double literals are recognised by the lexer when they contain `.` or `e`/`E`; NaN / infinity are refused at parse so the render-side `.expect(…)` stays load-bearing for compile-time literals and surfaces a runtime panic only for genuinely non-finite input field values. `uint64` / `fixed64` (range exceeds i64), repeated / map / message / enum fields, `bytes`, and `typed_search_attributes` remain rejected. See `docs/R7-BLOBLANG.md`. |
