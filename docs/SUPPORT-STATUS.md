...
| `search_attributes` | supported (slices 1 + 2 + 3a) | The empty-map `root = {}` (slice 1), non-empty literal maps `root = { "Key": <literal>, … }` (slice 2) with string / signed-integer / boolean values, and `this.<field>` references targeting singular `string` input fields (slice 3a) compile and flow through to `WorkflowStartOptions.search_attributes`. The field-ref encoder reads from the start path's `input` binding at call time. Non-string field refs, repeated/message fields, and richer Bloblang remain rejected (slice 3b). See `docs/R7-BLOBLANG.md`. |
