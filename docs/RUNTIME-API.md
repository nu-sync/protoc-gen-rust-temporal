...
| `worker::{WorkflowDefinition, ChildWorkflowOptions, StartedChildWorkflow, ChildWorkflowStartError}` | Re-exported from `temporalio-common` (`WorkflowDefinition`) and `temporalio-sdk` (the rest). The plugin's `workflows=true` emit produces `<RPC>Workflow` markers with `impl WorkflowDefinition` plus `start_<workflow>_child` helpers returning `StartedChildWorkflow<<RPC>Workflow>`. |
...
| `start_workflow_proto<I>` | workflow start, **non-Empty** input | `async fn(client, workflow_name: &'static str, workflow_id: &str, task_queue: &str, input: &I, id_reuse_policy: Option<WorkflowIdReusePolicy>, id_conflict_policy: Option<WorkflowIdConflictPolicy>, execution_timeout: Option<Duration>, run_timeout: Option<Duration>, task_timeout: Option<Duration>, enable_eager_workflow_start: bool, retry_policy: Option<RetryPolicy>) -> Result<WorkflowHandle>` where `I: TemporalProtoMessage` | 0.1.0; `enable_eager_workflow_start` + `id_conflict_policy` + `retry_policy` args added 0.1.2 |
| `start_workflow_proto_empty` | workflow start, **Empty** input | same as above without the `input` arg or `I` generic | 0.1.0; `enable_eager_workflow_start` + `id_conflict_policy` args added 0.1.2 |
...
| `update_with_start_workflow_proto<W, U, O>` | `<update>_with_start` free fn, **non-Empty** update output | `async fn(client, …, workflow_input: &W, update_name, update_input: &U, wait_policy, …, id_conflict_policy: Option<WorkflowIdConflictPolicy>) -> Result<(WorkflowHandle, O)>` | 0.1.3 |
| `update_with_start_workflow_proto_unit<W, U>` | `<update>_with_start` free fn, **Empty** update output | `async fn(client, …, workflow_input: &W, update_name, update_input: &U, wait_policy, …, id_conflict_policy: Option<WorkflowIdConflictPolicy>) -> Result<WorkflowHandle>` | 0.1.3 |
| `encode_search_attribute_string` / `_int` / `_bool` | R7 slice-2 building blocks for static search attributes. Plugin doesn't call them yet (slice 1 only models the empty map). Available for downstream wiring. | `fn(&str) -> Payload` / `fn(i64) -> Payload` / `fn(bool) -> Payload` | 0.1.2 |
| `encode_search_attribute_double` | IEEE-754 double encoder. Refuses NaN / infinity at the boundary (no valid JSON literal); whole-number doubles emit with the decimal point preserved (`1.0`, not `1`) so the wire shape stays unambiguous. Plugin doesn't call it yet; available for downstream consumers building float search attributes. | `fn(f64) -> Result<Payload>` | 0.1.6 |
| `decode_search_attribute_string` / `_int` / `_bool` | Inverses of the encoders above. Plugin doesn't call them (consumers reading server-supplied search attributes do). Validate the `json/plain` encoding and surface precise errors on mis-typed payloads. | `fn(&Payload) -> Result<String>` / `fn(&Payload) -> Result<i64>` / `fn(&Payload) -> Result<bool>` | 0.1.4 |
| `decode_search_attribute_double` | Inverse of `encode_search_attribute_double`. Refuses non-finite decoded values (server-supplied corruption guard). | `fn(&Payload) -> Result<f64>` | 0.1.6 |
| `encode_proto_payload<T>` / `decode_proto_payload<T>` | The bridge's internal helpers for building / validating `binary/protobuf` payloads against `WIRE-FORMAT.md`. Promoted to public so downstream tooling (custom dispatch layers, payload migrators, codec servers) can construct byte-identical payloads the generated client uses. `T: TemporalProtoMessage`. | `fn(&T) -> Payload` / `fn(&Payload) -> Result<T>` | 0.1.5 |
...
| `register_<service>_activities<I>` | `pub fn(&mut temporal_runtime::worker::Worker, I) -> &mut temporal_runtime::worker::Worker` where `I: <Service>Activities`. Wraps `I` in `Arc` and registers one typed callback per marker via `worker.register_activity_fn`. |

The trait method takes `&self`. The consumer's adapter (which wires the trait
to a `temporalio-sdk` Worker) is generated by the plugin: the helper clones an
`Arc<I>`, unwraps `TypedProtoMessage<T>` activity inputs, maps Empty inputs to
`()`, wraps non-Empty outputs in `TypedProtoMessage<T>`, and converts returned
errors with `temporal_runtime::worker::ActivityError::from`. Consumers no
longer need a parallel `#[activities]` impl just to adapt the generated trait.
...
| `temporal_runtime::worker::ActivityError` | bridge crate `worker` feature | Re-exported from `temporalio_sdk::activities::ActivityError`; used by generated callback adapters. |
